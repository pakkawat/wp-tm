...
// Start of functions ///////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if($this->booster_cachedir_autocleanup && $today['wday'] == 0)
			{
				if(is_dir($this->booster_cachedir))
				{
					...
					while(false !== ($file = readdir($handle)))
					{
						...
						{
							if (is_file($this->booster_cachedir . '/' . $file) && file_exists($this->booster_cachedir . '/' . $file)) {
                                try {
	...
	protected function getfilescontents($source = '',$type = '',$recursive = FALSE,$filescontent = '')
	{
		...
		if($type == 'css')
		{
			...
			for($i=0;$i<count($treffer[0]);$i++)
			{
                ...
                //check and remove any version numbers
                if($cparts = explode('?',$importfile)){
                    $importfile = $cparts[0];
                }
				...
				if(file_exists($importfile)) 
				{
                   // echo '@@@'.$importfile.'@@@@';
                    ...
                    // seem to need the extra ";" so it does not break things when replacing @import ...;
	...
	*/
	protected function css_datauri($filescontent = '',$dir = '')
	{
		...
		if($this->css_mhtml_enabled_ie)
		{
			...
			// Get Domainname
			((isset($_SERVER['SCRIPT_URI'])) ? $mhtmlhost = parse_url($_SERVER['SCRIPT_URI'], PHP_URL_HOST) : $mhtmlhost = $_SERVER['HTTP_HOST']);
            $mhtmlhost = filter_var($mhtmlhost, FILTER_SANITIZE_URL);
	...
	*/
	public function css()
	{
		...
		else
		{
			...
			if (is_file($interim_cachefile) && file_exists($interim_cachefile)) {
                try {
                    unlink($interim_cachefile);
                } catch(Exception $e) {
                    // error
                }
            }
	...
	*/
     ...
     * Max url length.
     */
    function max_url_length( $default = 2000 ) {
        $max_url_length = (int)$default >= 250 ? $default : 250;
        
        if (defined('GEODIR_GD_BOOSTER_MAX_URL_LENGTH') && GEODIR_GD_BOOSTER_MAX_URL_LENGTH >= 250 ) {
            $max_url_length = GEODIR_GD_BOOSTER_MAX_URL_LENGTH;
        }
	 ...
	 * CDN enabled.
	 */
	function cdn_enabled( $default = 0 ) {
		$cdn_enabled = (int)$default > 0 ? $default : 0;

		if (defined('GEODIR_GD_BOOSTER_CDN_ENABLED') && GEODIR_GD_BOOSTER_CDN_ENABLED > 0 ) {
			$cdn_enabled  = GEODIR_GD_BOOSTER_CDN_ENABLED;
		}
	 ...
	 * CDN root url.
	 */
	function cdn_root_url( $default = 0 ) {
		$cdn_root_url = (int)$default > 0 ? $default : 0;

		if (defined('GEODIR_GD_BOOSTER_CDN_ROOT_URL') && GEODIR_GD_BOOSTER_CDN_ROOT_URL != '' ) {
			$cdn_root_url = GEODIR_GD_BOOSTER_CDN_ROOT_URL;
		}
	 ...
	 * CDN root url.
	 */
	function cdn_file_ext( $default = 0 ) {
		$cdn_file_ext = (int)$default > 0 ? $default : 0;

		if (defined('GEODIR_GD_BOOSTER_CDN_FILE_EXT') && GEODIR_GD_BOOSTER_CDN_FILE_EXT != '' ) {
			$cdn_file_ext = GEODIR_GD_BOOSTER_CDN_FILE_EXT;
		}
