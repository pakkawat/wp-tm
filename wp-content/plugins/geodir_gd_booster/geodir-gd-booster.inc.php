		...
		class plugin extends share
		{
			...
			public function setup()
			{
				...
				$this->default_options = array(
					...
					'cdn_enabled'                    => '0', // `0|1`.
					'cdn_root_url'                   => '', // `string`.
					'cdn_file_ext'                   => 'bmp|bz2|gif|ico|gz|jpg|jpeg|mp3|pdf|png|rar|rtf|swf|tar|tgz|txt|wav|zip|js|css', // `string`.
				...
				) // Before the rewrite. Only if GDB was previously configured w/ options.
				{
					...
					if(!isset($options['cdn_enabled']) && isset($old_options['cdn_enabled']))
						$options['cdn_enabled'] = ((string)(integer)$old_options['cdn_enabled']) ? '0' : '1';

					if(!isset($options['cdn_root_url']) && isset($old_options['cdn_root_url']))
						$options['cdn_root_url'] = (string)$old_options['cdn_root_url'];

					if(!isset($options['cdn_file_ext']) && isset($old_options['cdn_file_ext']))
						$options['cdn_file_ext'] = (string)$old_options['cdn_file_ext'];
                    
                    if (!isset($options['max_url_length']) && isset($old_options['max_url_length']) && (int)$old_options['max_url_length'] >= 200) {
                        $options['max_url_length'] = (int)$old_options['max_url_length'];
                    } else {
                        $options['max_url_length'] = 2000;
                    }
				}
			...
			public function uninstall()
			{
				...
				if( is_multisite() ) { // Delete network options too.
					gd_booster_delete_site_options( array( __NAMESPACE__.'_options', __NAMESPACE__.'_notices', __NAMESPACE__.'_errors' ) );
				}
			...
			public function add_network_menu_pages()
			{
				...
				if ( is_multisite() && !gd_booster_is_plugin_active() ) {
					return;
				}
