...
function update_repeater_fields() {
    ...
    $('.at-time').each(function () {
        ...
        $this.timepicker({showSecond: false, timeFormat: format});
    ...
    $('.at-upload').delegate('.at-delete-file', 'click', function () {
        ...
        $.post(ajaxurl, {action: 'at_delete_file', data: data}, function (response) {
            response == '0' ? ( alert(geodir_all_js_msg.tax_meta_class_succ_del_msg), $parent.remove() ) : alert(geodir_all_js_msg.tax_meta_class_not_permission_to_del_msg);
        });
        ...
        $this.sortable({
            ...
            update: function () {
                ...
                $.post(ajaxurl, {action: 'at_reorder_images', data: data}, function (response) {
                    response == '0' ? alert(geodir_all_js_msg.tax_meta_class_order_save_msg) : alert(geodir_all_js_msg.tax_meta_class_not_permission_record_img_msg);
        ...
        jQuery(".mupload_img_holder").each(function (i, v) {
            if (jQuery(this).next().next().val() != '') {
                if (!jQuery(this).children().size() > 0) {
                    jQuery(this).append('<img src="' + jQuery(this).next().next().val() + '" style="max-height: 150px;max-width: 150px;" />');
