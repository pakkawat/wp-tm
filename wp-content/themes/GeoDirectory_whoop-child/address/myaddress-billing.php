<?php /* Template Name: myaddress-billing */ ?>
...
jQuery(document).ready(function($){
  ...
  $("form[name='billing_form']").validate({
    ...
    messages: {
      billing_address: "กรุณาเลือกที่อยู่ในการออกใบกำกับภาษี"
    },
...
<h1>ที่อยู่ในการออกใบกำกับภาษี ตามที่ได้ระบุไว้</h1>
...
<form name="billing_form">
  <table id="product_table" class="table table-bordred table-striped">
    ...
    <?php
      ...
      foreach ($arrAddress as $address) {
        ...
        echo '</td>';
        echo '<td><input type="radio" name="billing_address" value="'.$address->id.'" ';
  ...
  <input type="hidden" name="nonce" value="<?php echo wp_create_nonce( 'update_billing_address_' . $current_user->ID ); ?>"  />
